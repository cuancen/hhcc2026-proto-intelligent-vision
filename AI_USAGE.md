...
The vehicle's body paint material is dynamically overridden at runtime via our
Three.js frontend code (white paint; glass and emissive light parts keep their
...
no geometry was modified. This static glb asset is used solely for homepage
hero visual demo purposes, with an automatic fallback to our hand-written
Canvas wireframe cabin if loading fails. All Three.js scene setup, lighting,
shadow, hover-rotation animation logic and the wireframe fallback are fully
...
前端代码在运行时覆盖为白色（玻璃与车灯发光件保留原材质），并对文件做了
meshopt 几何压缩（24 MB → 5 MB，未改动几何）。GLB 静态资源仅用于首页视觉演示，
加载失败自动回退团队自研的 Canvas 线框；Three.js 场景、灯光、阴影与悬浮旋转
动画均为团队自研，未复用任何外部黑客松项目源码。页脚同步展示同一署名。

## Third-party dependencies (used as normal libraries)

- [React 18](https://react.dev/) / [Vite](https://vitejs.dev/) — MIT
- [three.js](https://threejs.org/) — MIT (landing hero car rendering;
  loaded via dynamic import, with wireframe fallback)
